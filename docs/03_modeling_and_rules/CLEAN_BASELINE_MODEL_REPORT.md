...
> **업데이트 (2026-08-26, 돈방별 scaler 적용 후)**: 아래 내용은 전체 데이터를 scaler 1개로 통합 정규화하던 시점의 기록입니다. 돈방별 scaler로 바꿔서 다시 실행한 결과, 이 보고서의 핵심 전제였던 "제외 row 26개(71408/chamber 1)"가 **더 이상 이상 후보로 잡히지 않습니다** (`bioenergy_split_v2` p97 confirmed anomaly 3개 → 0개). 즉 지금 다시 만들면 `excluded_rows: 0, clean_rows: 650`이 되어, 애초에 "정제할" 이상 구간이 없는 상태입니다. 예전에 이상으로 보였던 구간은 돈방 간 scaler 수준 차이로 인한 착시였을 가능성이 높습니다. 아래 절차/수치는 당시 기록으로 남겨둡니다.
